...
class SubTab(TS3LModule):
    ...
    def __init__(self,
        ...
        self.emb_dim = embedding_config.emb_dim if hasattr(
            embedding_config, "emb_dim") else 1

        if backbone_config.name == "mlp" and embedding_config.name == "feature_tokenizer":  # type: ignore
            self.column_idx = np.arange(
                self.input_dim * embedding_config.emb_dim)  # type: ignore
        ...
        if backbone_config.name == "mlp" and embedding_config.name == "feature_tokenizer":  # type: ignore
            ...
            self.n_overlap_dim = self.n_overlap_dim * \
                embedding_config.emb_dim  # type: ignore
        ...
        self.projector = Projector(
            self.backbone_module.output_dim, projection_dim)
        self.decoder = ShallowDecoder(
            self.backbone_module.output_dim, self.embedding_module.input_dim)
    ...
    def _second_phase_step(self,
        ...
        latent = latent.reshape(
            x.shape[0] // self.n_subsets, self.n_subsets, -1).mean(1)
    ...
    def __generate_noisy_xbar(self, x: torch.Tensor) -> torch.Tensor:
        ...
        if self.noise_type == "Swap":
            ...
            permutations = torch.stack([torch.randperm(no).reshape(
                (-1, 1)) for _ in range(dim)], dim=1).to(x.device)
            ...
            noise = torch.normal(
                mean=0.0, std=self.noise_level, size=x.shape, device=x.device)
            x_bar = x + noise
    ...
    def __generate_x_tilde(self, x: torch.Tensor, subset_column_idx: NDArray[np.int32]) -> torch.Tensor:
        ...
        mask = torch.bernoulli(torch.full(
            x_bar.shape, self.mask_ratio, device=x_bar.device))
    ...
    def __generate_subset(self,
        ...
        permuted_order = np.random.permutation(
            self.n_subsets) if self.shuffle else np.arange(self.n_subsets)

        subset_column_indice_list = [
            self.column_idx[:(self.subset_dim + self.n_overlap_dim)]]
        subset_column_indice_list.extend([self.column_idx[range(
            (i * self.subset_dim - self.n_overlap_dim), ((i + 1) * self.subset_dim))] for i in range(self.n_subsets)])
        ...
        if len(subset_column_indice) == 1:
            subset_column_indice = np.concatenate(
                [subset_column_indice, subset_column_indice])

        x_tildes = torch.concat([self.__generate_x_tilde(x, subset_column_indice[i]) for i in range(
            self.n_subsets)])  # [subset1, subset2, ... ,  subsetN]
